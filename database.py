...
class DataBase:
	...
	def new_universities_table(con, cur) -> None:
		...
		def get_majors() -> str:
			...
			for i in range(majors_count):
				...
				major_sex = int(input(f'Major {i} sex (0=Both, 1=Boys, 2=Girls): '))
		...
		use_majors_file = True if input(
			'Do you want to import a json file for majors data? ').lower() in ['y', 'yes', 1] else False
		...
		if use_majors_file:
			...
			unified_total = True if input('\nDo you want to use unified total combination? ').lower() in [
                            'y', 'yes', 1, ''] else False
		...
		# IF we get an error while creating the table then no data will be inserted to universities_data
		con.commit()
	...
	def drop_table(con, cur):
		...
		confirmation = True if input(
			f'Are you sure that you want to delete {en_name}-{year}-{semester} and all it\'s data?').lower() in ['y', 'yes', 1] else False
