...
# Localization things
app.config["LANGUAGES"] = {"ar": "العَرَبيَّة", "en": "English"}
RTL_LANGUAGES = ["ar"]
REGIONS = [
    ("MC", "Mecca Region"),
    ("RD", "Riyadh Region"),
...
def get_captcha() -> tuple:
    ...
    captcha_image = b64encode(image.generate(captcha_text, "jpeg").getvalue()).decode()
...
%s university has no hand in it🔴
"""
            ) % (university_data[4], university_data[5], university_data[2], university_data[1])
        )
        ...
        if not request.form.get("CAPTCHA") == session["CAPTCHA_TEXT"]:
            flash(gettext("Invalid CAPTCHA."), "error")
        ...
        try:
            ...
            flash(gettext("Form value error, "), "error")
            return redirect(url_for("home"))
        ...
        try:
            ...
            flash(
                gettext(
                    "Your data could not be inserted to the database,\
