...
class DataBase:
    ...
    def new_universities_table(con, cur) -> None:
        ...
        def get_total() -> List:
            ...
            Achievement = (
                float(input("Achievement percentage: ").removesuffix("%")) or None
            )
            ...
            return [
                None if not x else (x / 100 if 1 < x <= 100 else x) for x in combination
            ]
        ...
        choice = input("Do you want to import a json file for majors data? ".lower())
    ...
    def get_university_data(con, cur, id: str):
        ...
        cur.execute("""SELECT * FROM universities_data WHERE id = :id""", {"id": id})
        return cur.fetchone()
